...
이 강부터 "터미널에서 …" 라는 말이 계속 나옵니다. 터미널은 명령을 글자로 치는 검은 창입니다. 한 번 열어 두고 이 강 내내 씁니다.

::: windows
1. 키보드 **Windows 키**를 누르고 `powershell` 이라고 칩니다.
2. 검색 결과의 **Windows PowerShell** 을 클릭합니다(관리자 권한 아님).
3. 파란/검은 창이 뜨고 `PS C:\Users\내이름>` 처럼 끝이 `>` 로 끝나면 준비된 것입니다. 명령을 붙여 넣고 **Enter** 를 칩니다.

붙여 넣기는 창에서 **마우스 오른쪽 클릭**(또는 Ctrl+V). "터미널을 새로 연다"는 말은 이 창을 닫고 같은 방법으로 다시 여는 것입니다.
:::

::: mac
1. **command(⌘) + space** 를 누르고 `터미널` 이라고 칩니다.
2. **터미널** 앱을 Enter 로 엽니다.
3. 창에 `내이름@… ~ %` 처럼 끝이 `%` 로 끝나면 준비된 것입니다. 명령을 붙여 넣고(⌘+V) **return** 을 칩니다.

"터미널을 새로 연다"는 말은 이 창을 닫고(⌘+Q) 같은 방법으로 다시 여는 것입니다.
:::

폴더로 이동하는 명령은 `cd 폴더이름` 입니다. 예: 키트를 받은 뒤 `cd clay-episode-kit`.
...
2. Aside 앱을 한 번 열어 계정을 만들고 로그인합니다. 그러면 터미널용 `aside` 명령도 함께 설치됩니다. 터미널을 **새로 열고**(맨 위 「터미널 여는 법」— 이미 열려 있던 창은 닫고 다시 엽니다) 아래 명령을 붙여 넣고 Enter:
