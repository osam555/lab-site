...
export const COURSES: Course[] = [
  ...
  },
  {
    ...
    title: "영상 자동화",
    tagline: "클레이 애니 롱폼 2~3분 + 쇼츠 45초 + 썸네일 3종, 주제 하나로",
    description:
      "공개 키트 clay-episode-kit으로 대본 → Typecast 더빙 → 컷 계획 → Google Flow(Veo) 생성 → ffmpeg 조립 → 품질 게이트 → YouTube 업로드까지 한 편을 반자동으로 완성합니다. 예시 대본 「무지개는 왜 생길까」로 전 과정을 한 번 따라간 뒤, 내 주제·내 캐릭터로 바꿔 시리즈로 운영하는 법까지 8강으로 다룹니다.",
    level: "입문+",
    audience: "Claude Code를 설치해본 사람. 롱폼·쇼츠·썸네일을 매번 손으로 만드는 게 지친 사람",
    outcome: "내 주제·캐릭터로 채운 kit.config.json, 완성한 첫 편(롱폼+쇼츠+썸네일), 시리즈로 돌리는 큐·체크리스트",
    parts: ["1부 · 준비", "2부 · 첫 편 만들기", "3부 · 내 주제로 운영"],
    badge: "Windows · macOS",
    highlights: [
      "대본→더빙→컷 계획→Flow→조립→검사→업로드 6단계 파이프라인 한눈에",
      "Aside 브라우저로 Google Flow·YouTube Studio 자동 조작",
      "Typecast 더빙 + kit.config.json 하나로 캐릭터·브랜드·채널 고정",
      "컷 계획 자동 채점(cutplan_check) — Flow에 보내기 전에 실수 잡기",
      "완성 영상 자동 채점(qa_gate) — 8.0 미만이면 재작업",
      "썸네일 3종(아이·어른·클로즈업) 자동 생성·검사",
      "R2 임시 저장 → 큐 파일 → 드레이너로 유튜브 업로드",
      "내 주제로 대본·컷 계획 직접 쓰는 법 (PLANS 블록)",
      "시리즈 단위 제출·다운로드로 편당 대화 비용 10분의 1로",
      "강마다 스킬(/clay-episode)로 그 단계 바로 실행",
    ],
    relatedSkills: [
      "skill-system",
      "prompt-writing",
